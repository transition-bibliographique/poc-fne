...
const addClaim = (claims, property, value) => {
  ...
  if (_.isPlainObject(value) && (value.value == null || value.value === '')) return
...
const addSubfieldsClaims = (schema, claims, subfields, tag, tagIndex) => {
  subfields.map((subfield, index) => {
    const pseudoPropertyId = buildSubfieldPseudoId(schema, subfield, index, tag, tagIndex)
